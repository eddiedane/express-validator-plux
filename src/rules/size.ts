...
export function size({ data: [len] }: ValidatorFactionOptions<[number]>): ValidatorHandler {
  len = Number(len);
  return (val: any, meta) => {
    ...
    if (_valLen !== _len) {
      const msg =
        typeof val === "number"
          ...
          : typeof val === "string"
          ? `${sentenceCase(meta.path)} must be contain exactly ${_len} ${pluralize(
              "character",
              _len,
            )}`
          : `${sentenceCase(meta.path)} must contain exactly ${_len} ${pluralize("item", _len)}`;
